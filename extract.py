...
def extract_from_messages(messages: list[dict]) -> list[ExtractionResult]:
    ...
    prompt = (
        ...
        "IMPORTANT: If a message has a thumbs-up reaction (👍 or '👍' in the "
        "reactions metadata), treat any task extracted from that message as "
        "completed/done. Include 'completed: true' in the task's notes field. "
        "Example notes: 'Completed (thumbs-up reaction)'.\n\n"
    ...
    try:
        ...
        api_key = os.getenv("OPENAI_API_KEY") or os.getenv("OPENAI_ADMIN_KEY") or ""
        # LM Studio (and similar local servers) accept any non-empty string
        if not api_key:
            api_key = "sk-fake-key"  # dummy key accepted by LM Studio / local servers
